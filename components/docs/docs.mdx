...
より洗練された CSS-in-JS のソリューションを使いたい場合には、サーバーサイドレンダリングでスタイルを設定するための実装を追加する必要があるでしょう。We enable this by allowing you to define your own [custom `<Document>`](#custom-document) component that wraps each page.
