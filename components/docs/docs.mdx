...
コンポーネント`<Link>`もURLオブジェクトを受け取ることができ、URL文字列を作成するために自動でフォーマットします。
...
先程作成したファイルは、自動でデフォルト値をもとに構成されます。([カスタマイズ](https://www.typescriptlang.org/docs/handbook/compiler-options.html)もできます)

それから、`next dev`(または、`npm run dev`)を実行すると環境構築に必要なパッケージをインストールするようメッセージがでますのでインストールします。
...
> 注意点: Next.jsがnext-env.d.tsというファイルをプロジェクトのroot上に作成しますが、TypeScriptのコンパイラーが型を確実に選択するようにするためです。
>このファイルを削除することはできませんが、編集することはできます（ただし、削除する必要はありません）

> 注意点: Next.jsは、デフォルトではTypeScriptのstrictモードは有効ではないため、別途tsconfig.jsonにて設定する必要があります。
