...
`useFileSystemPublicRoutes` は単純にSSRでのファイル名に基づくルーティングを無効にします; クライエントサイドでのルーティングは
引き続きこれらのパスにアクセスします。もしこのオプションを使用するのなら、 あなたはプログラムによって不必要なルートへの誘導に対するガードをするべきです。

クライエントサイドでのファイル名に基づくルーティングにリダイレクトすることを禁じるようにクライエントサイドのルーターを設定したいと思うでしょう。
; その時は [Intercepting `popstate`](#intercepting-popstate)を参照してください。

#### 動的なアセットプレフィックス

時々、私たちは動的に`assetPrefix`を設定する必要があります。これは入ってくるリクエストに基づく`assetPrefix`を変更するときに役に立ちます。
