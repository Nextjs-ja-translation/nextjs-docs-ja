...
Next.js では、`NextPage` という型が提供されており、pages ディレクトリの中にあるページで使用することができます。`NextPage` は [`getInitialProps`](#fetching-data-and-component-lifecycle) に対して型情報を提供するため、自分で型定義をすることなく簡単に利用できます。
